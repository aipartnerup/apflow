...
async def test_llm_executor_completion(mock_litellm_module):
    ...
    mock_response.model_dump.return_value = {"choices": [{"message": {"content": "Hello world"}}]}
    mock_litellm_module.acompletion.return_value = mock_response

    executor = LLMExecutor()
    inputs = {"model": "gpt-4", "messages": [{"role": "user", "content": "Hi"}]}
...
async def test_llm_executor_real_api_call():
    ...
    if openai_key and any(m in openai_key for m in mock_keys):
        logger.warning(
            f"Detected potential mock key in OPENAI_API_KEY: {openai_key}. Ignoring for real API test."
        )
        openai_key = None
    if anthropic_key and any(m in anthropic_key for m in mock_keys):
        logger.warning(
            f"Detected potential mock key in ANTHROPIC_API_KEY: {anthropic_key}. Ignoring for real API test."
        )
        anthropic_key = None

    if not openai_key and not anthropic_key:
        pytest.skip(
            "No real API key found in environment (OPENAI_API_KEY or ANTHROPIC_API_KEY). "
