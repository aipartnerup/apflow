...
@pytest.mark.skipif(
    BatchCrewaiExecutor is None or CrewaiExecutor is None,
    reason="BatchCrewaiExecutor or CrewaiExecutor not available",
)
class TestBatchCrewaiExecutorExecution:
    ...
    async def test_execute_with_mock_crews(self):
        ...
        batch_crewai_executor = BatchCrewaiExecutor(
            works={
                "crew1": {
                    "agents": {
                        "agent1": {"role": "Agent 1", "goal": "Goal 1", "backstory": "Backstory 1"}
                    },
                    "tasks": {
                        "task1": {
                            "description": "Task 1",
                ...
                "crew2": {
                    "agents": {
                        "agent2": {"role": "Agent 2", "goal": "Goal 2", "backstory": "Backstory 2"}
                    },
                    "tasks": {
                        "task2": {
                            "description": "Task 2",
        ...
        with patch(
            "apflow.extensions.crewai.crewai_executor.CrewaiExecutor",
            side_effect=[mock_crewai_executor1, mock_crewai_executor2],
        ):
            # Execute batch
    ...
    async def test_execute_with_failure(self):
        ...
        mock_result1 = {"status": "success", "result": "Result from crew 1"}

        mock_result2 = {"status": "failed", "error": "Crew 2 failed", "result": None}
    ...
    def test_cancelable_property(self):
        ...
        batch_crewai_executor = BatchCrewaiExecutor(works={})
        assert (
            batch_crewai_executor.cancelable is True
        ), "BatchCrewaiExecutor should support cancellation"
    ...
    async def test_cancellation_preserves_token_usage_from_multiple_works(self):
        ...
        batch_crewai_executor = BatchCrewaiExecutor(
            works={
                ...
                "crew3": {
                    "agents": {
                        "agent3": {"role": "Agent 3", "goal": "Goal 3", "backstory": "Backstory 3"}
                    },
                    "tasks": {
                        "task3": {
                            "description": "Task 3",
        ...
        with patch("apflow.extensions.crewai.crewai_executor.CrewaiExecutor") as mock_crew_class:
            mock_crew_class.side_effect = [
                mock_crewai_executor1,
                mock_crewai_executor2,
                mock_crewai_executor3,
            ]
...
class TestBatchCrewaiExecutorRealExecution:
    ...
    async def test_execute_with_inputs(self):
        ...
        if result["status"] == "success":
            ...
            assert (
                greeting_has_name
                or farewell_has_name
                or (
                    isinstance(greeting_crew_result, dict)
                    and greeting_crew_result.get("status") == "success"
                    and isinstance(farewell_crew_result, dict)
                    and farewell_crew_result.get("status") == "success"
                )
