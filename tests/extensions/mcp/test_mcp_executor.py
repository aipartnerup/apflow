...
class TestMcpExecutor:
    ...
    async def test_execute_list_tools_stdio(self):
        ...
        with patch("asyncio.create_subprocess_exec") as mock_subprocess:
            ...
            mock_process.communicate = AsyncMock(
                return_value=(json.dumps(mock_response).encode("utf-8"), b"")
            )
    ...
    async def test_execute_list_resources_stdio(self):
        ...
        mock_response = {
            "jsonrpc": "2.0",
            "id": "mcp_123",
            "result": {"resources": [{"uri": "file:///path/to/file.txt", "name": "file.txt"}]},
        }
    ...
    async def test_execute_missing_transport(self):
        ...
        with pytest.raises(ValueError, match="transport is required"):
            await executor.execute({"operation": "list_tools"})
    ...
    async def test_execute_missing_operation(self):
        ...
        with pytest.raises(ValueError, match="operation is required"):
            await executor.execute({"transport": "stdio"})
    ...
    async def test_execute_missing_command_stdio(self):
        ...
        result = await executor.execute({"transport": "stdio", "operation": "list_tools"})
    ...
    async def test_execute_missing_url_http(self):
        ...
        with patch("apflow.extensions.mcp.mcp_executor.HTTPX_AVAILABLE", True):
            result = await executor.execute({"transport": "http", "operation": "list_tools"})
    ...
    async def test_execute_unsupported_transport(self):
        ...
        result = await executor.execute({"transport": "websocket", "operation": "list_tools"})
    ...
    async def test_execute_stdio_process_error(self):
        ...
        with patch("asyncio.create_subprocess_exec") as mock_subprocess:
            ...
            result = await executor.execute(
                {"transport": "stdio", "command": ["invalid_command"], "operation": "list_tools"}
            )
    ...
    async def test_execute_http_list_tools(self):
        ...
        mock_response = {
            "jsonrpc": "2.0",
            "id": "mcp_123",
            "result": {"tools": [{"name": "search_web", "description": "Search the web"}]},
        }

        with (
            patch("apflow.extensions.mcp.mcp_executor.HTTPX_AVAILABLE", True),
            ...
            result = await executor.execute(
                {"transport": "http", "url": "http://localhost:8000/mcp", "operation": "list_tools"}
            )
    ...
    async def test_execute_stdio_with_env_vars(self):
        ...
        mock_response = {"jsonrpc": "2.0", "id": "mcp_123", "result": {"tools": []}}
