...
def task_routes(use_test_db_session):
    ...
    return TaskRoutes(
        task_model_class=get_task_model_class(), verify_token_func=None, verify_permission_func=None
    )
...
async def in_progress_task(use_test_db_session):
    ...
    await task_repository.update_task(task_id=task_id, status="in_progress", progress=0.5)
...
async def task_tree_with_dependencies(use_test_db_session):
    ...
    return {"root_id": root_id, "child1_id": child1_id, "child2_id": child2_id}
...
class TestCriticalFieldValidation:
    ...
    async def test_update_parent_id_always_rejected(self, task_routes, mock_request, pending_task):
        ...
        params = {"task_id": pending_task, "parent_id": "new-parent-id"}
    ...
    async def test_update_user_id_always_rejected(self, task_routes, mock_request, pending_task):
        ...
        params = {"task_id": pending_task, "user_id": "new-user-id"}
        request_id = str(uuid.uuid4())
    ...
    async def test_update_dependencies_non_pending_rejected(
        ...
        params = {"task_id": completed_id, "dependencies": [{"id": root_id, "required": True}]}
        request_id = str(uuid.uuid4())
    ...
    async def test_update_dependencies_in_progress_rejected(
        ...
        await task_repository.update_task(
            task_id=in_progress_id, status="in_progress", progress=0.5
        )

        # Try to update dependencies for in_progress task
        params = {"task_id": in_progress_id, "dependencies": [{"id": root_id, "required": True}]}
        request_id = str(uuid.uuid4())
    ...
    async def test_update_dependencies_invalid_reference(
        ...
        assert "Dependency reference 'non-existent-task-id' not found for user 'test_user'" in str(
            exc_info.value
        )
    ...
    async def test_update_dependencies_not_list_rejected(
        ...
        params = {"task_id": pending_task, "dependencies": "not-a-list"}
        request_id = str(uuid.uuid4())
...
class TestOtherFieldsUpdate:
    ...
    @pytest.mark.asyncio
    async def test_update_inputs_from_any_status(self, task_routes, mock_request, completed_task):
        """Test that inputs can be updated from completed status"""
        params = {"task_id": completed_task, "inputs": {"new_key": "new_value"}}
        request_id = str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_update_name_from_any_status(self, task_routes, mock_request, completed_task):
        """Test that name can be updated from completed status"""
        params = {"task_id": completed_task, "name": "New Task Name"}
        request_id = str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_update_priority_from_any_status(self, task_routes, mock_request, completed_task):
        """Test that priority can be updated from completed status"""
        params = {"task_id": completed_task, "priority": 5}
        request_id = str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_update_params_from_any_status(self, task_routes, mock_request, completed_task):
        """Test that params can be updated from completed status"""
        params = {"task_id": completed_task, "params": {"executor_id": "new_executor"}}
        request_id = str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_update_schemas_from_any_status(self, task_routes, mock_request, completed_task):
        """Test that schemas can be updated from completed status"""
        params = {"task_id": completed_task, "schemas": {"input_schema": {"type": "object"}}}
        request_id = str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_update_status_from_any_status(self, task_routes, mock_request, completed_task):
        """Test that status can be updated from completed status"""
        params = {"task_id": completed_task, "status": "pending"}
        request_id = str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_update_result_from_any_status(self, task_routes, mock_request, pending_task):
        """Test that result can be updated from pending status"""
        params = {"task_id": pending_task, "result": {"output": "test_result"}}
        request_id = str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_update_error_from_any_status(self, task_routes, mock_request, pending_task):
        """Test that error can be updated from pending status"""
        params = {"task_id": pending_task, "error": "Test error message"}
        request_id = str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_update_progress_from_any_status(self, task_routes, mock_request, pending_task):
        """Test that progress can be updated from pending status"""
        params = {"task_id": pending_task, "progress": 0.75}
        request_id = str(uuid.uuid4())
...
class TestMultipleFieldsUpdate:
    ...
    @pytest.mark.asyncio
    async def test_update_multiple_fields_success(self, task_routes, mock_request, pending_task):
    ...
    @pytest.mark.asyncio
    async def test_update_with_critical_field_error(self, task_routes, mock_request, pending_task):
...
class TestDependencyUpdateEdgeCases:
    ...
    async def test_update_dependencies_empty_list(
        ...
        params = {"task_id": task_tree_with_dependencies["child1_id"], "dependencies": []}
        request_id = str(uuid.uuid4())
