...
class TestMcpServer:
    ...
    async def test_handle_request_tools_call(self, server):
        ...
        mock_tool_result = {"content": [{"type": "text", "text": "test result"}]}
        ...
        with patch.object(server.tools_registry, "call_tool", new_callable=AsyncMock) as mock_call:
            ...
            request = {
                ...
                "id": 1,
                "method": "tools/call",
                "params": {"name": "execute_task", "arguments": {"task_id": "test-id"}},
            }

            result = await server.handle_request(request)

            assert result == mock_tool_result
            mock_call.assert_called_once_with("execute_task", {"task_id": "test-id"}, None)
    ...
    async def test_handle_request_resources_read(self, server):
        ...
        mock_resource_result = {
            "contents": [{"uri": "task://test-id", "mimeType": "application/json", "text": "{}"}]
        }

        with patch.object(
            server.resources_registry, "read_resource", new_callable=AsyncMock
        ) as mock_read:
    ...
    async def test_handle_request_tools_call_missing_name(self, server):
        ...
        request = {"jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": {"arguments": {}}}
    ...
    async def test_handle_request_resources_read_missing_uri(self, server):
        ...
        request = {"jsonrpc": "2.0", "id": 1, "method": "resources/read", "params": {}}
