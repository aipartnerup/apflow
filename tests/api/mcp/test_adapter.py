...
class TestTaskRoutesAdapter:
    ...
    async def test_execute_task_with_task_id(self, adapter):
        ...
        with patch.object(
            adapter.task_routes, "handle_task_execute", new_callable=AsyncMock
        ) as mock_execute:
            mock_execute.return_value = mock_result

            result = await adapter.execute_task(
                {"task_id": "test-task-id", "request_id": "test-request-id"}
            )
    ...
    async def test_execute_task_with_tasks_array(self, adapter):
        ...
        mock_result = {"success": True, "root_task_id": "test-root-id"}
    ...
    async def test_create_task_single(self, adapter):
        ...
        with patch.object(
            adapter.task_routes, "handle_task_create", new_callable=AsyncMock
        ) as mock_create:
    ...
    async def test_create_task_multiple(self, adapter):
        ...
        with patch.object(
            ...
            result = await adapter.create_task(
                {"tasks": [{"name": "task-1"}, {"name": "task-2"}], "request_id": "test-request-id"}
            )
    ...
    async def test_get_task(self, adapter):
        ...
        with patch.object(
            adapter.task_routes, "handle_task_get", new_callable=AsyncMock
        ) as mock_get:
    ...
    async def test_update_task(self, adapter):
        ...
        with patch.object(
            adapter.task_routes, "handle_task_update", new_callable=AsyncMock
        ) as mock_update:
            mock_update.return_value = mock_result

            result = await adapter.update_task(
                {"task_id": "test-task-id", "name": "updated-task", "request_id": "test-request-id"}
            )
    ...
    async def test_delete_task(self, adapter):
        ...
        with patch.object(
            adapter.task_routes, "handle_task_delete", new_callable=AsyncMock
        ) as mock_delete:
    ...
    async def test_list_tasks(self, adapter):
        """Test listing tasks"""
        mock_result = {"tasks": [{"id": "task-1"}, {"id": "task-2"}], "total": 2}

        with patch.object(
            adapter.task_routes, "handle_tasks_list", new_callable=AsyncMock
        ) as mock_list:
            mock_list.return_value = mock_result

            result = await adapter.list_tasks(
                {"status": "pending", "limit": 10, "request_id": "test-request-id"}
            )
    ...
    async def test_get_task_status(self, adapter):
        ...
        mock_result = {"task_id": "test-task-id", "status": "running", "progress": 0.5}

        with patch.object(
            adapter.task_routes, "handle_running_tasks_status", new_callable=AsyncMock
        ) as mock_status:
    ...
    async def test_cancel_task(self, adapter):
        ...
        mock_result = {"success": True, "task_id": "test-task-id", "status": "cancelled"}

        with patch.object(
            adapter.task_routes, "handle_task_cancel", new_callable=AsyncMock
        ) as mock_cancel:
