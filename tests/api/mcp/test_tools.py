...
class TestMcpToolsRegistry:
    ...
    async def test_call_tool_execute_task(self, registry):
        """Test calling execute_task tool routes through call_by_tool_name"""
        mock_result = {"success": True, "root_task_id": "test-root-id"}

        with patch.object(
            registry.adapter, "call_by_tool_name", new_callable=AsyncMock
        ) as mock_call:
            mock_call.return_value = mock_result

            result = await registry.call_tool("execute_task", {"task_id": "test-task-id"})

            assert "content" in result
            assert len(result["content"]) > 0
            assert result["content"][0]["type"] == "text"
            mock_call.assert_called_once_with("execute_task", {"task_id": "test-task-id"}, None)
    ...
    async def test_call_tool_create_task(self, registry):
        ...
        with patch.object(
            ...
            result = await registry.call_tool("create_task", {"tasks": [{"name": "test-task"}]})
    ...
    async def test_call_tool_get_task(self, registry):
        ...
        with patch.object(
            ...
            result = await registry.call_tool("get_task", {"task_id": "test-task-id"})
    ...
    async def test_call_tool_update_task(self, registry):
        ...
        with patch.object(
            ...
            result = await registry.call_tool(
                "update_task", {"task_id": "test-task-id", "name": "updated-task"}
            )
    ...
    async def test_call_tool_delete_task(self, registry):
        ...
        with patch.object(
            ...
            result = await registry.call_tool("delete_task", {"task_id": "test-task-id"})
    ...
    async def test_call_tool_list_tasks(self, registry):
        ...
        with patch.object(
            ...
            result = await registry.call_tool("list_tasks", {"status": "pending"})
    ...
    async def test_call_tool_get_task_status(self, registry):
        ...
        with patch.object(
            ...
            result = await registry.call_tool("get_task_status", {"task_id": "test-task-id"})
    ...
    async def test_call_tool_cancel_task(self, registry):
        ...
        with patch.object(
            ...
            result = await registry.call_tool("cancel_task", {"task_id": "test-task-id"})
