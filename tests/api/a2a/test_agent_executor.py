...
from a2a.types import Message, DataPart, Task, TaskState
...
class TestAgentExecutor:
    ...
    def executor(self):
        ...
        task_routes = TaskRoutes(
            task_model_class=TaskModel, verify_token_func=None, verify_permission_func=None
        )

        return AIPartnerUpFlowAgentExecutor(task_routes=task_routes, verify_token_func=None)
    ...
    async def test_extract_tasks_wrapped_format(self, executor):
        ...
        tasks = [
            {"id": "task-1", "user_id": "test-user", "name": "Task 1", "status": "pending"},
            {"id": "task-2", "user_id": "test-user", "name": "Task 2", "status": "pending"},
        ]
    ...
    async def test_extract_tasks_direct_format(self, executor):
        ...
        tasks = [
            {"id": "task-1", "user_id": "test-user", "name": "Task 1"},
            {"id": "task-2", "user_id": "test-user", "name": "Task 2"},
        ]
    ...
    async def test_execute_simple_mode(self, executor, mock_event_queue):
        ...
        with (
            patch("apflow.api.a2a.agent_executor.get_default_session") as mock_get_session,
            ...
            if result.artifacts and len(result.artifacts) > 0:
                artifact_data = (
                    result.artifacts[0].parts[0].root.data if result.artifacts[0].parts else None
                )
                if artifact_data:
    ...
    @pytest.mark.asyncio
    async def test_execute_simple_mode_from_copy(
        self, executor, mock_event_queue, use_test_db_session, sync_db_session
    ):
        """Test simple mode execution with from_copy scenario"""
        # Create original task in real DB
        repo = TaskRepository(sync_db_session)
        original_task = await repo.create_task(
            id="original-task-id", name="Original Task", user_id="test-user", status="completed"
        )

        metadata = {"from_copy": True, "task_id": original_task.id, "_recursive": False}
        context = self._create_request_context([], metadata=metadata)

        with patch.object(executor.task_executor, "execute_tasks") as mock_execute_tasks:
            # execute_tasks needs to return a result referencing the copied task's root id
            async def fake_execute_tasks(tasks, root_task_id, **kwargs):
                # tasks is from from_copy output_list — find the copied root
                copied_root_id = tasks[0]["id"] if tasks else root_task_id
                return {
                    "status": "completed",
                    "progress": 1.0,
                    "root_task_id": copied_root_id,
                    "original_task_id": original_task.id,
                }

            mock_execute_tasks.side_effect = fake_execute_tasks

            result = await executor._execute_simple_mode(context, mock_event_queue)

        assert mock_execute_tasks.called
        assert result is not None
        assert result.metadata["root_task_id"] is not None

    @pytest.mark.asyncio
    async def test_execute_simple_mode_from_mixed(
        self, executor, mock_event_queue, use_test_db_session, sync_db_session
    ):
        ...
        repo = TaskRepository(sync_db_session)
        original_task = await repo.create_task(
            id="original-mixed-id", name="Original Task", user_id="test-user", status="completed"
        )

        metadata = {"from_mixed": True, "task_id": original_task.id, "_recursive": True}
        context = self._create_request_context([], metadata=metadata)
    ...
    @pytest.mark.asyncio
    async def test_execute_simple_mode_from_link(
        self, executor, mock_event_queue, use_test_db_session, sync_db_session
    ):
        """Test simple mode execution with from_link scenario"""
        # Create original task in real DB — from_link requires completed status
        repo = TaskRepository(sync_db_session)
        original_task = await repo.create_task(
            id="original-link-id", name="Original Task", user_id="test-user", status="completed"
        )

        metadata = {"from_link": True, "task_id": original_task.id}
        context = self._create_request_context([], metadata=metadata)
    ...
    @pytest.mark.asyncio
    async def test_execute_simple_mode_from_archive(
        self, executor, mock_event_queue, use_test_db_session, sync_db_session
    ):
        """Test simple mode execution with from_archive scenario"""
        # Create original task in real DB — from_archive requires completed status
        repo = TaskRepository(sync_db_session)
        original_task = await repo.create_task(
            id="original-archive-id", name="Original Task", user_id="test-user", status="completed"
        )

        metadata = {"from_archive": True, "task_id": original_task.id}
        context = self._create_request_context([], metadata=metadata)
    ...
    @pytest.mark.asyncio
    async def test_execute_streaming_mode_from_copy(
        self, executor, mock_event_queue, use_test_db_session, sync_db_session
    ):
        ...
        repo = TaskRepository(sync_db_session)
        original_task = await repo.create_task(
            id="original-stream-id", name="Original Task", user_id="test-user", status="completed"
        )
    ...
    async def test_execute_system_resource_monitoring_with_executor(
        self, use_test_db_session, sync_db_session, mock_event_queue
    ):
        """
        ...
        tasks = [
            {
                ...
                "schemas": {"input_schema": {}},  # Optional: can define input schema
                "params": {
            ...
            },
            {
                ...
                "has_children": False,
                "dependencies": [],
                "schemas": {"method": "system_info_executor"},  # Executor id
                "inputs": {"resource": "cpu"},
            },
            {
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_execute_system_resource_monitoring(
        self, use_test_db_session, sync_db_session, mock_event_queue
    ):
        """
        ...
        with patch("apflow.api.a2a.agent_executor.get_default_session") as mock_get_session_agent:
            ...
            assert (
                len(post_hook_calls) >= 4
            ), f"Expected at least 4 post-hook calls, got {len(post_hook_calls)}"
            ...
            for call in post_hook_calls:
                assert (
                    call["task_status"] == "completed"
                ), f"Task {call['task_id']} should be completed"
            ...
            logger.info(
                f"==Pre-hook calls: {len(pre_hook_calls)}==\n{json.dumps(pre_hook_calls, indent=2)}"
            )
            logger.info(
                f"==Post-hook calls: {len(post_hook_calls)}==\n{json.dumps(post_hook_calls, indent=2)}"
            )
    ...
    def _create_cancel_context(
        self, task_id: str = None, context_id: str = None, metadata: dict = None
    ) -> RequestContext:
    ...
    async def test_cancel_with_task_id(self, executor, mock_event_queue):
        ...
        cancel_result = {"status": "cancelled", "message": "Task cancelled successfully"}

        with (
            patch("apflow.api.a2a.agent_executor.get_default_session") as mock_get_session,
            patch.object(
                executor.task_executor, "cancel_task", new_callable=AsyncMock
            ) as mock_cancel_task,
        ):
            mock_get_session.return_value = Mock()
            ...
            mock_cancel_task.assert_called_once_with(
                task_id=task_id, error_message=None, db_session=mock_get_session.return_value
            )
    ...
    async def test_cancel_with_context_id(self, executor, mock_event_queue):
        ...
        with (
            ...
            mock_cancel_task.assert_called_once_with(
                task_id=context_id, error_message=None, db_session=mock_get_session.return_value
            )
    ...
    async def test_cancel_with_custom_error_message(self, executor, mock_event_queue):
        ...
        context = self._create_cancel_context(
            task_id=task_id, metadata={"error_message": error_message}
        )

        cancel_result = {"status": "cancelled", "message": error_message}
    ...
    async def test_cancel_with_partial_result(self, executor, mock_event_queue):
        ...
        cancel_result = {
            "status": "cancelled",
            "message": "Task cancelled successfully",
            "result": {"partial_data": "some result", "progress": 0.5},
        }
    ...
    async def test_cancel_with_force_flag(self, executor, mock_event_queue):
        ...
        context = self._create_cancel_context(task_id=task_id, metadata={"force": True})
    ...
    async def test_cancel_with_all_fields(self, executor, mock_event_queue):
        ...
        cancel_result = {
            "status": "cancelled",
            "message": "Task cancelled successfully",
            "token_usage": {"total_tokens": 2000, "prompt_tokens": 1000, "completion_tokens": 1000},
            "result": {"partial_data": "result data", "progress": 0.75},
    ...
    def _create_request_context_with_method(
        self, method: str, params: dict = None, metadata: dict = None
    ) -> RequestContext:
    ...
    async def test_execute_with_tasks_create_method(self, executor, mock_event_queue):
        """Test execute with tasks.create method raises ValueError (CRUD not supported via A2A)"""
        context = self._create_request_context_with_method(
            method="tasks.create", params={"name": "Test Task", "user_id": "test-user"}
        )

        with pytest.raises(ValueError, match="not supported via A2A message/send"):
    ...
    async def test_execute_with_tasks_get_method(self, executor, mock_event_queue):
        """Test execute with tasks.get method raises ValueError (CRUD not supported via A2A)"""
        context = self._create_request_context_with_method(
            method="tasks.get", params={"task_id": "task-123"}
        )
    ...
    async def test_execute_with_tasks_list_method(self, executor, mock_event_queue):
        """Test execute with tasks.list method raises ValueError (CRUD not supported via A2A)"""
        context = self._create_request_context_with_method(
            method="tasks.list", params={"user_id": "test-user", "limit": 10}
        )
    ...
    async def test_execute_with_tasks_execute_method(self, executor, mock_event_queue):
        ...
        context = self._create_request_context_with_method(
            method="tasks.execute", params={"task_id": "task-123"}
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_with_unknown_method_defaults_to_execution(
        self, executor, mock_event_queue
    ):
        """Test execute with no method defaults to task execution (backward compatibility)"""
        context = self._create_request_context_with_method(method=None, metadata={})
    ...
    @pytest.mark.asyncio
    async def test_execute_crud_method_error_handling(self, executor, mock_event_queue):
        """Test that CRUD methods via A2A raise ValueError with redirect message"""
        context = self._create_request_context_with_method(
            method="tasks.get", params={"task_id": "nonexistent"}
        )
    ...
    async def test_execute_with_tasks_update_method(self, executor, mock_event_queue):
        """Test execute with tasks.update method raises ValueError (CRUD not supported via A2A)"""
        context = self._create_request_context_with_method(
            method="tasks.update", params={"task_id": "task-123", "name": "Updated Task"}
        )
    ...
    async def test_execute_with_tasks_delete_method(self, executor, mock_event_queue):
        """Test execute with tasks.delete method raises ValueError (CRUD not supported via A2A)"""
        context = self._create_request_context_with_method(
            method="tasks.delete", params={"task_id": "task-123"}
        )
    ...
    async def test_execute_with_tasks_detail_method(self, executor, mock_event_queue):
        """Test execute with tasks.detail method raises ValueError (CRUD not supported via A2A)"""
        context = self._create_request_context_with_method(
            method="tasks.detail", params={"task_id": "task-123"}
        )
    ...
    async def test_execute_with_tasks_tree_method(self, executor, mock_event_queue):
        """Test execute with tasks.tree method raises ValueError (CRUD not supported via A2A)"""
        context = self._create_request_context_with_method(
            method="tasks.tree", params={"task_id": "task-123"}
        )
    ...
    async def test_execute_with_tasks_children_method(self, executor, mock_event_queue):
        """Test execute with tasks.children method raises ValueError (CRUD not supported via A2A)"""
        context = self._create_request_context_with_method(
            method="tasks.children", params={"parent_id": "task-123"}
        )
    ...
    async def test_execute_with_tasks_running_list_method(self, executor, mock_event_queue):
        """Test execute with tasks.running.list method raises ValueError (not supported via A2A)"""
        context = self._create_request_context_with_method(
            method="tasks.running.list", params={"user_id": "test-user"}
        )
    ...
    async def test_execute_with_tasks_running_status_method(self, executor, mock_event_queue):
        """Test execute with tasks.running.status method raises ValueError (not supported via A2A)"""
        context = self._create_request_context_with_method(
            method="tasks.running.status", params={"task_ids": ["task-1", "task-2"]}
        )
    ...
    async def test_execute_with_tasks_running_count_method(self, executor, mock_event_queue):
        """Test execute with tasks.running.count method raises ValueError (not supported via A2A)"""
        context = self._create_request_context_with_method(
            method="tasks.running.count", params={"user_id": "test-user"}
        )
    ...
    async def test_execute_with_tasks_copy_method(self, executor, mock_event_queue):
        """Test execute with tasks.clone method raises ValueError (CRUD not supported via A2A)"""
        context = self._create_request_context_with_method(
            method="tasks.clone", params={"task_id": "task-123", "recursive": True}
        )
    ...
    async def test_execute_with_skill_id_mapping(self, executor, mock_event_queue):
        """Test execute with skill_id for CRUD raises ValueError (not supported via A2A)"""
        context = self._create_request_context_with_method(
            method=None, metadata={"skill_id": "tasks.create"}
        )

        # skill_id=tasks.create is extracted as method by adapter, then rejected
        with pytest.raises(ValueError, match="not supported via A2A message/send"):
            await executor.execute(context, mock_event_queue)
