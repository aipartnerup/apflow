...
class TestConfigCommand:
    ...
    def test_config_list_command(self):
        ...
        # Set some values
        runner.invoke(app, ["config", "set", "key1", "value1"])
        runner.invoke(app, ["config", "set", "key2", "value2"])
    ...
    def test_config_list_json_format(self):
        ...
        result = runner.invoke(app, ["config", "list", "-f", "json"])
        assert result.exit_code == 0
    ...
    def test_config_unset_command(self):
        ...
        # First set a value
        runner.invoke(app, ["config", "set", "key_to_delete", "value"])
    ...
    def test_config_init_server_default(self):
        ...
        result = runner.invoke(app, ["config", "init-server"])
        assert result.exit_code == 0
        assert "✅" in result.stdout
    ...
    def test_config_init_server_custom_url(self):
        ...
        result = runner.invoke(app, ["config", "init-server", "--url", "http://prod-server.com:8000"])
        assert result.exit_code == 0
        assert "✅" in result.stdout
    ...
    def test_config_token_masking(self):
        ...
        # Set a token
        runner.invoke(app, ["config", "set", "api_auth_token", "secret-token-12345"])
        
        # Get it - should be masked (only first char + ...)
        result = runner.invoke(app, ["config", "get", "api_auth_token"])
        assert result.exit_code == 0
        assert "***" in result.stdout
    ...
    def test_config_list_shows_masked_tokens(self):
        ...
        runner.invoke(app, ["config", "set", "api_auth_token", "secret-token"])
