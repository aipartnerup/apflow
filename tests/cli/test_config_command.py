...
def isolated_config_dir(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        cli_config, "get_all_cli_config_locations", mock_get_all_cli_config_locations
    )
    ...
    monkeypatch.setattr(
        cli_config, "get_all_cli_config_locations", original_get_all_cli_config_locations
    )
...
class TestConfigCommand:
    ...
    def test_config_set_admin_auth_token(self):
        ...
        result = runner.invoke(cli, ["config", "set", "admin_auth_token", "test-token-xyz"])
        assert result.exit_code == 0
        assert "✅" in result.stdout

    def test_config_set_api_token_alias(self):
        """Test that api-token alias maps to admin_auth_token."""
        result = runner.invoke(cli, ["config", "set", "api-token", "test-token-xyz"])
        assert result.exit_code == 0
        assert "✅" in result.stdout
...
class TestInitServerEnvSync:
    ...
    def test_init_server_generates_secret_when_env_not_set(
        self, tmp_path, monkeypatch, isolated_config_dir
    ):
    ...
    def test_init_server_generates_secret_when_no_env_file(
        self, tmp_path, monkeypatch, isolated_config_dir
    ):
    ...
    def test_init_server_env_secret_syncs_with_existing_config(
        self, tmp_path, monkeypatch, isolated_config_dir
    ):
