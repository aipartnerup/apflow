...
class TestMigration:
    ...
    def test_migrate_json_to_yaml(self):
        """Test migration from JSON files to YAML."""
        with TemporaryDirectory() as tmpdir:
            config_dir = Path(tmpdir)
            legacy_config = config_dir / "config.json"
            legacy_secrets = config_dir / "secrets.json"
            yaml_config = config_dir / "config.cli.yaml"

            with patch("apflow.cli.cli_config.get_config_dir", return_value=config_dir):
                with patch("apflow.cli.cli_config.get_cli_config_file_path", return_value=yaml_config):
                    ...
                    secrets_data = {
                        "api_auth_token": "old-token-123",
                        "jwt_secret": "secret-123",
                    }

                    with open(legacy_config, "w") as f:
                        json.dump(config_data, f)
                    with open(legacy_secrets, "w") as f:
                        json.dump(secrets_data, f)

                    # Migrate
                    migrated = migrate_json_to_yaml()

                    # Verify migration
                    assert migrated is not None
                    assert migrated["api_server_url"] == "http://localhost:8000"
                    assert migrated["admin_auth_token"] == "old-token-123"  # Renamed
                    assert migrated["jwt_secret"] == "secret-123"
                    assert "api_timeout" not in migrated  # Deprecated field removed
                    ...
                    assert (config_dir / "secrets.json.bak").exists()

                    # Verify YAML file can be loaded
                    with patch("apflow.cli.cli_config.get_all_cli_config_locations", return_value=[yaml_config]):
                        loaded = load_cli_config()
                        assert loaded["api_server_url"] == "http://localhost:8000"
                        assert loaded["admin_auth_token"] == "old-token-123"

    def test_migrate_only_config_json(self):
        """Test migration with only config.json (no secrets.json)."""
        with TemporaryDirectory() as tmpdir:
            config_dir = Path(tmpdir)
            legacy_config = config_dir / "config.json"
            yaml_config = config_dir / "config.cli.yaml"
            ...
            with patch("apflow.cli.cli_config.get_config_dir", return_value=config_dir):
                with patch("apflow.cli.cli_config.get_cli_config_file_path", return_value=yaml_config):
                    ...
                    config_data = {"api_server_url": "http://localhost:8000"}
    ...
    def test_migrate_only_secrets_json(self):
        """Test migration with only secrets.json (no config.json)."""
        with TemporaryDirectory() as tmpdir:
            config_dir = Path(tmpdir)
            legacy_secrets = config_dir / "secrets.json"
            yaml_config = config_dir / "config.cli.yaml"
    ...
    def test_migrate_no_legacy_files(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with patch("apflow.cli.cli_config.get_config_dir", return_value=config_dir):
                with patch("apflow.cli.cli_config.get_cli_config_file_path", return_value=yaml_config):
                    # Migrate (should return None)
                    migrated = migrate_json_to_yaml()

                    # Should return None (no migration needed)
                    assert migrated is None

    def test_migrate_skips_if_yaml_exists(self):
        """Test that migration is skipped if YAML file already exists."""
        with TemporaryDirectory() as tmpdir:
            config_dir = Path(tmpdir)
            legacy_config = config_dir / "config.json"
            yaml_config = config_dir / "config.cli.yaml"
            ...
            with patch("apflow.cli.cli_config.get_config_dir", return_value=config_dir):
                with patch("apflow.cli.cli_config.get_cli_config_file_path", return_value=yaml_config):
                    # Create YAML file first (use localhost to avoid jwt_secret requirement)
                    existing_config = {"api_server_url": "http://localhost:8000"}
                    save_cli_config_yaml(existing_config)

                    # Create legacy JSON file
                    with open(legacy_config, "w") as f:
                        json.dump({"api_server_url": "http://legacy:8000"}, f)
