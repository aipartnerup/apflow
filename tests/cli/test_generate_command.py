...
class TestGenerateCommand:
    ...
    def test_generate_command_help(self):
        ...
        assert (
            "Generate task trees from natural language requirements" in result.stdout
            or "task-tree" in result.stdout
        )
    ...
    def test_generate_task_tree_help(self):
        ...
        assert (
            "max-tokens" in result.stdout
            or "max_tokens" in result.stdout
            or "temperature" in result.stdout
    ...
    def test_generate_missing_api_key(self, monkeypatch, use_test_db_session):
        ...
        with patch("apflow.cli.commands.generate.os.getenv", return_value=None):
            with patch("os.environ.get", return_value=None):
                result = runner.invoke(cli, ["generate", "task-tree", "Test requirement"])
    ...
    async def test_generate_basic(
        self, mock_llm_api_key, mock_generated_tasks, use_test_db_session
    ):
        ...
        with patch(
            "apflow.cli.commands.generate.get_default_session", return_value=use_test_db_session
        ):
            with patch("apflow.cli.commands.generate.TaskRepository", return_value=mock_repository):
                with patch(
                    "apflow.cli.commands.generate.TaskExecutor", return_value=mock_task_executor
                ):
                    result = runner.invoke(
                        cli, ["generate", "task-tree", "Fetch data from API and process it"]
                    )
        ...
        with patch(
            ...
            with patch("apflow.cli.commands.generate.TaskRepository", return_value=mock_repository):
                with patch(
                    ...
                    result = runner.invoke(
                        cli, ["generate", "task-tree", "Test requirement", "--temperature", "0.9"]
                    )
        ...
        with patch(
            ...
            with patch("apflow.cli.commands.generate.TaskRepository", return_value=mock_repository):
                with patch(
                    ...
                    result = runner.invoke(
                        cli, ["generate", "task-tree", "Test requirement", "--max-tokens", "6000"]
                    )
    ...
    @pytest.mark.asyncio
    async def test_generate_with_output_file(
        self, mock_llm_api_key, mock_generated_tasks, use_test_db_session
    ):
        ...
        with patch(
            ...
            with patch("apflow.cli.commands.generate.TaskRepository", return_value=mock_repository):
                with patch(
                    ...
                    try:
                        result = runner.invoke(
                            cli, ["generate", "task-tree", "Test requirement", "--output", tmp_path]
                        )
    ...
    @pytest.mark.asyncio
    async def test_generate_with_save(
        self, mock_llm_api_key, mock_generated_tasks, use_test_db_session
    ):
        ...
        with patch(
            ...
            with patch("apflow.cli.commands.generate.TaskRepository", return_value=mock_repository):
                with patch(
                    ...
                    with patch(
                        "apflow.core.execution.task_creator.TaskCreator",
                        return_value=mock_task_creator,
                    ):
                        result = runner.invoke(
                            cli, ["generate", "task-tree", "Test requirement", "--save"]
                        )
    ...
    def test_generate_parameter_validation(self):
        ...
        result = runner.invoke(
            cli, ["generate", "task-tree", "Test requirement", "--temperature", "invalid"]
        )
        ...
        result = runner.invoke(
            cli, ["generate", "task-tree", "Test requirement", "--max-tokens", "invalid"]
        )
        # Should fail with parameter error
        assert result.exit_code != 0
