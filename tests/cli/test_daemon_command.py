...
class TestDaemonCommand:
    ...
    def test_daemon_start_mocked(self, mock_popen, cleanup_daemon_files):
        ...
        result = runner.invoke(
            cli, ["daemon", "start", "--port", "9001", "--protocol", "mcp", "--background"]
        )
    ...
    @patch("apflow.cli.commands.daemon.is_process_running")
    @patch("apflow.cli.commands.daemon.os.kill")
    def test_daemon_stop_with_sigkill_fallback_mocked(
        self, mock_kill, mock_is_running, cleanup_daemon_files
    ):
...
class TestDaemonCommandIntegration:
    ...
    def test_daemon_start_real(self, cleanup_daemon_files):
        ...
        result = runner.invoke(cli, ["daemon", "start", "--port", "9999", "--background"])
    ...
    def test_daemon_start_with_protocol_real(self, cleanup_daemon_files):
        ...
        result = runner.invoke(
            cli, ["daemon", "start", "--port", "9997", "--protocol", "mcp", "--background"]
        )
    ...
    def test_daemon_restart_real(self, cleanup_daemon_files):
        ...
        result = runner.invoke(cli, ["daemon", "start", "--port", "9998", "--background"])

        if result.exit_code == 0:
            result = runner.invoke(cli, ["daemon", "restart", "--port", "9998"])
