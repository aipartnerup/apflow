...
class TestCustomTaskModelWithAgentExecutor:
    ...
    async def test_execute_with_custom_task_model_and_hooks(
        ...
        try:
            sync_db_session.execute(
                text(f"ALTER TABLE {TASK_TABLE_NAME} ADD COLUMN project_id VARCHAR(100)")
            )
            sync_db_session.commit()
        except Exception:
            ...
            post_hook_calls.append(
                {
                    ...
                    "has_pre_hook_marker": (
                        inputs.get("_pre_hook_executed", False) if inputs else False
                    ),
        ...
        with patch("apflow.api.a2a.agent_executor.get_default_session") as mock_get_session_agent:
            ...
            assert hasattr(
                task_after, "project_id"
            ), "Custom TaskModel should have project_id field"
            assert (
                task_after.project_id == project_id
            ), f"Expected project_id={project_id}, got {task_after.project_id}"
            ...
            assert (
                len(post_hook_calls) == 1
            ), f"Expected 1 post-hook call, got {len(post_hook_calls)}"
            ...
            assert (
                post_call["has_pre_hook_marker"] is True
            ), "Post-hook should see pre-hook modifications"
            ...
            logger.info(
                f"Task result keys: {list(task_after.result.keys()) if isinstance(task_after.result, dict) else 'N/A'}"
            )
            ...
            assert (
                post_call["has_pre_hook_marker"] is True
            ), "Pre-hook modifications should be visible to post-hook"
