...
class HeavyImportDetector(ast.NodeVisitor):
            ...
            for alias in node.names:
                ...
                if module_name in HEAVY_DEPENDENCIES:
                    self.violations.append(
                        (node.lineno, module_name, HEAVY_DEPENDENCIES[module_name])
                    )
        self.generic_visit(node)
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        ...
        if self.in_function == 0 and node.module:  # Module level
            ...
            if module_name in HEAVY_DEPENDENCIES:
                self.violations.append((node.lineno, module_name, HEAVY_DEPENDENCIES[module_name]))
        self.generic_visit(node)
    ...
    if all_violations:
        ...
        for file_path, violations in all_violations:
            ...
            for lineno, module, description in violations:
                ...
                print("    💡 Fix: Move import inside function or use lazy loading")
            print()
