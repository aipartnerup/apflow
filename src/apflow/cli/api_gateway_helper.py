...
def should_use_api() -> bool:
    ...
    if not cm.is_api_configured():
        ...
        try:
            ...
            logger.debug("Default API auto-check failed for %s: %s", default_url, discover_exc)
    ...
    if cm.is_api_configured():
        ...
        try:
            is_accessible = run_async_safe(cm.check_api_server_accessible(url, timeout=3.0))
...
async def api_with_fallback_decorator(
    ...
    try:
        ...
        cm = get_config_manager()
        if cm.use_local_db:
            logger.warning(f"API call failed, falling back to local DB: {e}")
            return await fallback_func
        else:
