...
@app.command()
def flow(
    executor_id: Optional[str] = typer.Argument(
        None, help="Executor ID to execute (deprecated: use --tasks instead)"
    ),
    tasks: Optional[str] = typer.Option(
        None, "--tasks", "-t", help="Tasks JSON array (list of task objects)"
    ),
    ...
    inputs: Optional[str] = typer.Option(
        None, "--inputs", "-i", help="Input JSON string (legacy: use with executor_id)"
    ),
    inputs_file: Optional[Path] = typer.Option(
        None, "--inputs-file", help="Input JSON file (legacy: use with executor_id)"
    ),
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="Output file path"),
    user_id: str = typer.Option("cli_user", "--user-id", "-u", help="User ID for task execution"),
    background: bool = typer.Option(
        False, "--background", "-b", help="Run in background (returns immediately with task ID)"
    ),
    watch: bool = typer.Option(
        False, "--watch", "-w", help="Watch task status in real-time after starting"
    ),
    use_demo: bool = typer.Option(
        False, "--use-demo", help="Use demo mode (returns demo data instead of executing)"
    ),
):
    """
    ...
    try:
        ...
        if tasks_file:
            ...
            typer.echo(
                "Error: Either --tasks/--tasks-file or executor_id must be provided", err=True
            )
            raise typer.Exit(1)
        ...
        async def execute_task_group(
            task_group: List[Dict[str, Any]], group_index: int
        ) -> Dict[str, Any]:
            ...
            try:
                ...
                logger.error(
                    f"Task group {group_index + 1} execution failed: {str(e)}", exc_info=True
                )
                return {"status": "failed", "error": str(e), "root_task_id": None}
                ...
                def create_status_table():
                    ...
                    for root_task_id in root_task_ids:
                        ...
                        if task:
                            ...
                            progress_str = (
                                f"{float(task.progress) * 100:.1f}%" if task.progress else "0.0%"
                            )
        ...
        try:
            ...
            if len(all_results) == 1:
                ...
                result = {
                    "status": (
                        "completed"
                        if all(r.get("status") == "completed" for r in all_results)
                        else "partial"
                    ),
            ...
            result = {"status": "failed", "error": str(e), "result": None}
