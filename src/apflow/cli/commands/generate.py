...
def task_tree(
    ...
    user_id: Optional[str] = typer.Option(
        None, "--user-id", "-u", help="User ID for the generated tasks"
    ),
    ...
    temperature: Optional[float] = typer.Option(
        None, "--temperature", "-t", help="LLM temperature (0.0 to 2.0, default: 0.7)"
    ),
    max_tokens: Optional[int] = typer.Option(
        None, "--max-tokens", help="Maximum tokens in LLM response (default: 4000)"
    ),
    ...
    try:
        ...
        if api_key:
            ...
            if not api_key:
                ...
                typer.echo("\nExample .env file:\n  OPENAI_API_KEY=sk-your-key-here", err=True)
                raise typer.Exit(1)
