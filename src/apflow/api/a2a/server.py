...
from apflow.api.capabilities import get_all_agent_skills
...
# Load skills from capabilities registry at startup
logger.info("Loading skills from capabilities registry...")
all_skills = get_all_agent_skills()
logger.info(f"Loaded {len(all_skills)} skills successfully")
...
def _create_request_handler(
    ...
    agent_executor = AIPartnerUpFlowAgentExecutor(
        task_routes=task_routes, verify_token_func=verify_token_func
    )
...
def generate_token(
    payload: Dict[str, Any], secret_key: str, algorithm: str = "HS256", expires_in_days: int = 30
) -> str:
    """
