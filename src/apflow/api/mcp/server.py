...
class McpServer:
    ...
    async def handle_request(
        self, request: Dict[str, Any], http_request: Optional[Request] = None
    ) -> Dict[str, Any]:
        """
        ...
        if method == "tools/list":
            return {"tools": self.tools_registry.list_tools()}
            ...
            result = await self.tools_registry.call_tool(tool_name, arguments, http_request)
            return result

        elif method == "resources/list":
            return {"resources": self.resources_registry.list_resources()}
            ...
            result = await self.resources_registry.read_resource(uri, http_request)
            return result
