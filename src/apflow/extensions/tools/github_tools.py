...
class GitHubAnalysisTool(BaseTool):
    ...
    description: str = (
        "Analyze GitHub presence, repositories, and metrics for a project or organization"
    )
                    ...
                    if include_repos:
                        ...
                        if self.session:
                            async with self.session.get(repos_url) as repos_response:
                                ...
                                total_stars = sum(
                                    repo.get("stargazers_count", 0) for repo in repos_data
                                )
                                total_forks = sum(repo.get("forks_count", 0) for repo in repos_data)
                                total_watchers = sum(
                                    repo.get("watchers_count", 0) for repo in repos_data
                                )

                                # Find most popular repo
                                most_popular_repo = (
                                    max(repos_data, key=lambda x: x.get("stargazers_count", 0))
                                    if repos_data
                                    else None
                                )
                                ...
                                result.update(
                                    {
                                        ...
                                        "most_popular_repo": (
                                            {
                                                "name": (
                                                    most_popular_repo.get("name", "")
                                                    if most_popular_repo
                                                    else ""
                                                ),
                                                "stars": (
                                                    most_popular_repo.get("stargazers_count", 0)
                                                    if most_popular_repo
                                                    else 0
                                                ),
                                                "language": (
                                                    most_popular_repo.get("language", "")
                                                    if most_popular_repo
                                                    else ""
                                                ),
                                            }
