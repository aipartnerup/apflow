...
class GenerateExecutor(BaseTask):
    ...
    def _build_llm_prompt(self, requirement: str, user_id: Optional[str] = None) -> str:
        ...
        prompt_parts = [
            ...
            "merged into the task's inputs by the framework (task_manager.py).",
            "",
            "CrewAI tasks support variable substitution using {variable_name} syntax in task descriptions.",
            "The crew.kickoff(inputs=...) receives these inputs and performs variable substitution.",
            "",
            "✓ CORRECT Way - Use Meaningful Variable Names:",
            "  Use descriptive variable names that match the dependency's output:",
            "  Example:",
            ...
            "          'tasks': {",
            "            'analyze': {",
            "              'description': 'Analyze this website content: {website_content}',",
            "              'agent': 'analyst',",
            "              'prompt': 'Return JSON analysis with insights and recommendations',",
            ...
            "",
            "  The framework will:",
            "  1. Execute 'scrape-website' task → gets result (e.g., HTML content)",
            "  2. Merge result into 'analyze-content' inputs as {'scrape-website': result}",
            "  3. Pass to crewai_executor → crew.kickoff(inputs={'scrape-website': result, ...})",
            "  4. CrewAI substitutes {website_content} if the result contains 'website_content' key",
            "",
            "✓ BEST PRACTICE:",
            "  - Use generic variable names: {content}, {data}, {website_content}, {api_response}",
            "  - Do NOT use task IDs as variable names (e.g., {scrape-website})",
            "  - The dependency result structure determines available variable names",
            "  - For scrape_executor: result may contain 'content', 'text', 'html' keys",
            "  - For rest_executor: result may contain 'body', 'data', 'response' keys",
            "",
            "❌ WRONG - Using Task IDs:",
            "  Don't do: 'description': 'Analyze: {scrape-task-e29b-41d4}'",
            "  This won't work unless the result explicitly has that key.",
            "",
